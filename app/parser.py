...
async def get_upwork_jobs(bot: Bot, config: Settings):
    ...
    logger.info("Sending request to Upwork API")
    logger.info("Sending request to Upwork API")
    logger.info(f"Request URL: {url}")
    logger.info(f"Request Headers: {params}")

    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as response:
            logger.info("Received response from Upwork API")
    ...
    for feed in feeds.entries:
        logger.info("Processing feed entry: %s", feed.title)
        ...
        logger.info("Metadata parsed: %s", data)
        ...
        thId = 197
        if "Full Stack Development" in data["category"]:
            thId = 174
        elif "Mobile App Development" in data["category"] or "Android" in data["category"] or "Flutter" in data["category"]:
            thId = 324
